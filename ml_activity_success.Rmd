...
We have selected three models to tune: 1) Support Vector Machine (SVM) with a [Radial Basis Function (RBF) kernel][4], 2) [RandomForest][5], and 3) [Stochastic Gradient Boost Machine][7]. To improve SVM accuracy we doubled the tune length for this model. For good measure, we also include an [Ensemble][8] model of the three algorithms to obtain better predictive performance.
...
myGrd = expand.grid(n.trees = c(myTrees, myTrees+15, myTrees+30),
                    interaction.depth = c(MyDepth-1, MyDepth, MyDepth+1),
                    shrinkage = c(myShrnk-0.025, myShrnk, myShrnk+0.025),
...
resamps <- resamples(list(
             ...
             RandomForest    = tune.rft2,
             GradientBoost   = tune.gbm2
           ))
summary(resamps)
...
cm.svr <- confusionMatrix(trn.vld$classe, yhat.svr)
cm.rft <- confusionMatrix(trn.vld$classe, yhat.rft)
cm.gbm <- confusionMatrix(trn.vld$classe, yhat.gbm)
```
...
set.seed(123)
# Create dataframe with the predictions of the separate models, combined with the classe variable (from the validation set)
...
cm.stk <- confusionMatrix(trn.tst$classe, yhat.stk)
```
...
```{r eval_stk, echo=TRUE, eval=TRUE, cache=TRUE, collapse=TRUE}
# Confusion Matrix
...
The ensemble model's training accuracy (`r round(getTrainPerf(fit.stack)$TrainAccuracy * 100, 2)`%) shows a `r round((getTrainPerf(fit.stack)$TrainAccuracy - getTrainPerf(fit.svr)$TrainAccuracy) * 100, 2)`% predictive improvement over the SVM model (`r round(getTrainPerf(fit.svr)$TrainAccuracy * 100, 2)`%). Against the test data, the ensemble model accuracy (`r round(confusionMatrix(trn.tst$classe, yhat.stk)[[3]]["Accuracy"] * 100, 2)`%) is similar to the training accuracy. This means that the model is well generalized and therefore not overfit to the training data. Let's see how it compares to the separate models on the test data:

* SVM-Radial: `r round(confusionMatrix(trn.tst$classe, yhat.svr)[[3]]["Accuracy"] * 100, 2)`%
* Random Forest: `r round(confusionMatrix(trn.tst$classe, yhat.rft)[[3]]["Accuracy"] * 100, 2)`%
* Gradient Boost: `r round(confusionMatrix(trn.tst$classe, yhat.gbm)[[3]]["Accuracy"] * 100, 2)`%

Although the ensemble model performs better overall, it's performance by class variable has improved as well:

```{r stats_stk, echo=TRUE, eval=TRUE, cache=TRUE, collapse=TRUE}
...
pred.acc <- rbind(confusionMatrix(yhat.svr, trn.tst$classe)$byClass[, 1],
                  confusionMatrix(yhat.rft, trn.tst$classe)$byClass[, 1],
                  confusionMatrix(yhat.gbm, trn.tst$classe)$byClass[, 1],
                  confusionMatrix(yhat.stk, trn.tst$classe)$byClass[, 1])
row.names(pred.acc) <- c("SVM.Radial", "Random.Forest", "Gradient.Boost", "Ensemble.Stack")

# Class Summary
kable(pred.acc, caption = "Predictive Accuracy by Class:")
