...
myMNode = tune.gbm2$bestTune$n.minobsinnode; myMNode
...
Different algorithms will find some variables more predictve than others, and will rank them differently. This shows up in the variables of importances (see below). The ensemble model is able to capitalize on those differences to boost predictive performance. 

After fitting the separate models to our validation set, we created modelled predictors so that we can train the ensemble model. Then we assess the model performance against our test set. This "stacked" model should have improved predictive performance over the separate models.
...
Although the Gradient Boost model doesn't perform as well as the other separate models, the stacked model is able to learn from its variance to boost performance over all of the separate models. 
